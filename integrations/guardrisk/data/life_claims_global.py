...
def prepare_life_claims_payload(data: list, start_date: date, end_date: date):
    ...
    for claim in data:
        ...
        try:
            ...
            log.debug(f"Invalid JSON data: {e}")
        else:
            claim_details = {
                "TimeStamp": timestamp,
                ...
                "ReasonForRepudiation": claim.get("repudiation_reason", ""),  # TODO add repudiation_reason to claim
                "ClaimRepudiated": claim.get("claim_repudiated", "N"),  # TODO add field boolean
